      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1033'">
            <xsl:text>FSU Jena - AlGe</xsl:text>
          </xsl:when>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1060'">
            <xsl:text>Slog Chicago</xsl:text>
          </xsl:when>
        </xsl:choose>
      </xsl:when>
...
<xsl:value-of select="$displayAuthor" />
						...
						<xsl:if test="string-length($volume) > 0 or string-length($pages) > 0">
							...
							<xsl:if test="string-length($pages) > 0"><xsl:text>, S. </xsl:text><xsl:value-of select="$pages"/></xsl:if>
</xsl:if>

						<xsl:if test="/b:Citation/b:PageSuffix">
							<xsl:value-of select="/b:Citation/b:PageSuffix"/>
						</xsl:if>
<xsl:choose><xsl:when test="not(/b:Citation/b:LastAuthor)"><xsl:text>; </xsl:text></xsl:when><xsl:otherwise><xsl:text>.</xsl:text></xsl:otherwise></xsl:choose>
			...
			<xsl:when test="b:Bibliography">
                ...
                <xsl:variable name="theCapLongPrefixProducedBy">
								...
								<xsl:text>- </xsl:text>
<xsl:value-of select="normalize-space(b:Author/b:Author/b:NameList/b:Person/b:Last)"/>
<xsl:text> </xsl:text>
<xsl:value-of select="normalize-space(b:Year)"/>
<xsl:text> = </xsl:text>

<!-- Voller Eintrag: Nachname, Vorname, Titel, Ort Jahr -->
<xsl:variable name="L_last" select="normalize-space(b:Author/b:Author/b:NameList/b:Person/b:Last)"/>
<xsl:variable name="L_first" select="normalize-space(b:Author/b:Author/b:NameList/b:Person/b:First)"/>
<xsl:variable name="L_title" select="normalize-space(b:Title)"/>
<xsl:variable name="L_city" select="normalize-space(b:City)"/>
<xsl:variable name="L_year" select="normalize-space(b:Year)"/>

<xsl:value-of select="$L_last"/>
<xsl:if test="$L_first != ''">
  <xsl:text>, </xsl:text><xsl:value-of select="$L_first"/>
</xsl:if>
<xsl:if test="$L_title != ''">
  <xsl:text>, </xsl:text><xsl:value-of select="$L_title"/>
</xsl:if>
<xsl:if test="$L_city != ''">
  <xsl:text>, </xsl:text><xsl:value-of select="$L_city"/>
</xsl:if>
<xsl:if test="$L_year != ''">
  <xsl:text> </xsl:text><xsl:value-of select="$L_year"/>
</xsl:if>
<xsl:text>.</xsl:text>
							</xsl:element>
						</xsl:for-each>
